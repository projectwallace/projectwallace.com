...
<FilterGroup>
	...
	<FilterOption
		name="font-size-sorting"
		value="by-source-order"
		bind:group={sorting}
	>
		Sort by source order
	</FilterOption>
	<FilterOption name="font-size-sorting" value="by-size" bind:group={sorting}
		>Sort by size</FilterOption
	>
	<FilterOption name="font-size-sorting" value="by-count" bind:group={sorting}
		>Sort by count</FilterOption
	>
	<FilterOption
		name="font-size-sorting"
		value="alphabetical"
		bind:group={sorting}>Sort A-Z</FilterOption
	>
</FilterGroup>
	...
	{#each sorted as [value, locations] (value)}
		<li class="size coverable">
			...
			<button
				...
				aria-selected={selected_item?.value === value &&
				selected_item.type === "font-size"
					? "true"
					: "false"}
				onclick={(event) => {
